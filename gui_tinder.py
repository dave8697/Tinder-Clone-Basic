...
class TinderGUI:
    def __init__(self):
        ...
        self.lblWelcome = Label(self.root, width=25,
                                height=2, bg='Red', font=('Arial', 15, 'bold'))
    ...
    def addUser(self, child, name, gender, city, email, password):
        result = self.tinderBackend.userRegister(
    ...
    def showUsers(self):
        ...
        for user in userList:
            if user[0] != self.tinderBackend.currentUserId:
                ...
                lID = Label(child, text=user[0], width=10, height=2, font=(
                    'Arial', 15)).grid(row=i, column=0)
                lName = Label(child, text=user[1], width=15, height=2, font=(
                    'Arial', 15)).grid(row=i, column=1)
                lGender = Label(child, text=user[2], width=15, height=2, font=(
                    'Arial', 15)).grid(row=i, column=2)
                lCity = Label(child, text=user[3], width=15, height=2, font=(
                    'Arial', 15)).grid(row=i, column=3)
                q = self.check(user[0])
                if q:
    ...
    def sendProposal(self, userid):
        ...
        if k:
            messagebox.showinfo(
                'Success', 'Yaay! Proposal Sent! Fingers Crossed!!')
        else:
    ...
    def viewSent(self):
        ...
        lWelcome = Label(child, text='Sent Proposals', width=40, height=3,
                         font=('courier', 15, 'bold'), bg='Yellow').grid(row=0, column=0, columnspan=4)
        for user in uList:
            lID = Label(child, text=user[3], width=10, height=2, font=(
                'Arial', 15)).grid(row=i, column=0)
            lName = Label(child, text=user[4], width=15, height=2, font=(
                'Arial', 15)).grid(row=i, column=1)
            lGender = Label(child, text=user[5], width=15, height=2, font=(
                'Arial', 15)).grid(row=i, column=2)
            lCity = Label(child, text=user[6], width=15, height=2, font=(
                'Arial', 15)).grid(row=i, column=3)
            i = i+1
